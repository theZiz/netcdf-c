...
static struct LEGALMODES {
    ...
    const int checkable; /* Can we use NC_check_file_type on this path? */
} legalmodes[] = {
/* Format X Implementation X iosp*/
{"netcdf-3",NC_FORMAT_CLASSIC,NC_FORMATX_NC3,0,1},
{"classic",NC_FORMAT_CLASSIC,NC_FORMATX_NC3,0,1},
{"netcdf-4",NC_FORMAT_NETCDF4,NC_FORMATX_NC4,0,1},
{"enhanced",NC_FORMAT_NETCDF4,NC_FORMATX_NC4,0,1},
...
url_getmodel(const char* modestr, NCmode* model)
{
	...
	for(;legal->tag;legal++) {
		...
		break; /* stop search of legal modes */
	    }
        }
    }	
